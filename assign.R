...
setwd("/Users/joxntex/Desktop/R_assignment/week3")
...
print("minimun women height")
data <- read.csv("Women.csv")
print(min(data$height))
print("maximun women height")
data <- read.csv("Women.csv")
...
df<- women$height
save(df,file = "height.RData")
print(df)
df<- women$weight
save(df,file = "weight.RData")
print(df)
...
df <- data.frame(height = c(get(load("height.RData"))),weight= c(get(load("weight.RData"))))
print(df)
...
data <- data.frame(height = c(get(load("height.RData"))),weight= c(get(load("weight.RData"))))
newdata <- data[ which(data$height > 65),]
print(newdata)
...
dim(data.frame(height = c(get(load("height.RData"))),weight= c(get(load("weight.RData")))))
